...
export async function signin(req, res) {
  try {
    ...
    const { nickName, isAuth, id } = response;
    const ID = response.userID;
    const token = tokenGenerater(nickName, ID, isAuth);
    console.log(token);
    res.status(200).json({ nickName, isAuth, ID, id, token });
  } catch (err) {
...
export async function refresh(req, res) {
  try {
    ...
    jwt.verify(refreshToken, config.jwt.refreshToken, (error, decoded) => {
      if (error) {
        res.sendStatus(401);
      } else {
        console.log(decoded);
...
export async function getMyPostings(req, res) {
  try {
    const userID = req.params.id;
    const postings = await userRepository.readMyPostings(userID);
    if (!postings) {
