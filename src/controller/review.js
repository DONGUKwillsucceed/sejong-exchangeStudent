...
export async function postData(req, res){
  ...
  try{
      const body = req.body;
      const country = req.params.country;
      const queryResult = await reviewRepository.postOne(body, country);
      res.status(200).json(queryResult);
...
export async function putData(req, res){
  ...
  try{
      const id = req.params.id;
      const body = req.body;
      const queryResult = await reviewRepository.putOne(id, body);
      res.status(200).json(queryResult);
...
export async function removeData(req, res){
  ...
  try{
      const id = req.params.id;
      const queryResult = await reviewRepository.removeOne(id);
      res.status(200).json(queryResult);
